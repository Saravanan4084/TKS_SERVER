...
NON_BROADCAST_COMMANDS = {"hi", "say", "comp"}
    ...
    if command_type(command) == "Normal":
        ...
        if check_permissions(accountid, command):
            if reply is not None:
                bs.broadcastmessage(reply, transient=True, clients=[clientid])
            try:
